...
def scraped_by_mpn(driver, vendor_id, vendor_url, product_id, brand_name_from_DB, mpn):
    try:
        ...
        search_btn = WebDriverWait(driver, 20).until(EC.element_to_be_clickable((By.CSS_SELECTOR, "div.search-wrapper.search-user-block")))
        search_btn.click()

        # Wait for input
        search_input = WebDriverWait(driver, 20).until(EC.visibility_of_element_located((By.CSS_SELECTOR, "input[type='search']")))
        search_input.clear()
        ...
        try:
            result = WebDriverWait(driver, 5).until(EC.presence_of_element_located((By.CSS_SELECTOR, "div#snize-search-results-list-mode a.snize-view-link")))
            product_url = result.get_attribute("href")
        except:
            if current_url.rstrip("#") != vendor_url:
            # if current_url != search_url:
                logger.info("Product opened directly.")
                product_url = current_url
            else:
                logger.info(f"No product found for MPN: {mpn}")
                VendorWebsiteScrapingMPNStatus(mpn, vendor_id, product_id)
                return None, None
        ...
        try: 
            info = driver.find_element(By.CSS_SELECTOR,"div.woocommerce-info").text.strip().lower()
            if "no products" in info:
                logger.info(f"No product found for MPN: {mpn}")
                VendorWebsiteScrapingMPNStatus(mpn, vendor_id, product_id)
                return None, None
        except:
        ...
        try:
            MainDiv = WebDriverWait(driver, 20).until(EC.element_to_be_clickable((By.CSS_SELECTOR, 'main#main')))
        except:
        ...
        try:
            brand_text = driver.find_element(
                By.CSS_SELECTOR,
        ...
        if sku and brand_text:
            sku_clean = sku

            # Remove brand prefix (e.g. LOC--, LOC-, AO-, etc.)
            prefixes = [
                brand_text,
                brand_text[:3],   # aos, loc
            ]
        ...
        if sku_clean and mpn and sku_clean.lower() == mpn.lower():
            mpn = sku_clean
        else:
            logger.info(f"SKU '{sku_clean}' does not match MPN '{mpn}' ")
            return None, None        

        if sku_clean is None:
            return None, None
                
        product_name = MainDiv.find_element(By.CSS_SELECTOR, 'h1.product_title').text.strip()

        images = []
        try:
            img_elements = MainDiv.find_elements(By.CSS_SELECTOR, 'main#main img.wp-post-image')
        ...
        try:
            msrp_el = MainDiv.find_element(By.CSS_SELECTOR, 'div.product-view div.product__price div.ComparePrice strike')
            # print(msrp_el)
        ...
        try:
            price_el = MainDiv.find_element(By.CSS_SELECTOR, 'p.price')
            # print(price_el)
        ...
        stock_text = None
        try:
            stock_text_element = driver.find_element(By.CSS_SELECTOR,'main#main p.stock')
        ...
        try:
            el = driver.find_element(By.CSS_SELECTOR, 'main#main p.stock')
        ...
        try:               
            ...
            # Click Add to Cart
            add_to_cart = wait.until(EC.element_to_be_clickable((By.CSS_SELECTOR, "button.single_add_to_cart_button ")))
            driver.execute_script("arguments[0].click();", add_to_cart)
            logger.debug("Add to Cart button clicked")

            wait.until(EC.visibility_of_element_located((By.CSS_SELECTOR, 'a.added_to_cart')))
            time.sleep(5)

            # Wait for Calculate Shipping button after AJAX update
            shipping_button = wait.until(EC.presence_of_element_located((By.CSS_SELECTOR, 'a.added_to_cart ')))
            ...
            try:
                shipping_element = wait.until(EC.visibility_of_element_located((By.CSS_SELECTOR, 'tr.woocommerce-shipping-totals.shipping span.woocommerce-Price-amount.amount')))
            ...
            remove_button = wait.until(EC.element_to_be_clickable((By.CSS_SELECTOR, "a.remove")))
            driver.execute_script("arguments[0].click();", remove_button)
            logger.debug("Product removed from cart")

        except Exception as e:
            logger.exception(f"Error finding or clicking button")
            pass

        temp["product_mpn"] = mpn
        temp["vendor_sku"] = mpn
        ...
        temp["description_text"] = None
        temp["description_html"] = None
        temp["specifications_html"] = None
        temp["vendorprice_stock_text"] = stock_text
        temp["vendorprice_stock"] = vendorprice_stock
        temp["msrp"] = msrp
        temp['product_pdfs'] = None
...
def getMpns(driver, vendor_id):
    ...
    try:
        ...
        if conn.is_connected():
                ...
                WHERE 
                    (
                        EXISTS (
                            ...
                            AND TVP1.vendorprice_date BETWEEN DATE_SUB(CURDATE(), INTERVAL 0 DAY) 
                            AND CURDATE()
                        )
                        OR EXISTS (
                            SELECT 1
            ...
            not_in_clause = ""
            if product_ids:
                not_in_clause = f"AND Product.product_id NOT IN ({','.join(product_ids)})"
            ...
            getVendorURLQuery = f"""
                ...
                WHERE ProductVendor.vendor_id = 10021
                AND ErpData.atp > 0 
                AND material_grp LIKE '%AOSPAR%'
                # AND Brand.brand_id IN (157,756,722,4,763,762,6,730,8,724,740,211,15)
                # {not_in_clause}
                # AND gprule_group_name_parent_id IN (514)
                #AND Brand.brand_type = 'Plumbing'
                AND Product.product_id NOT IN ({','.join(product_ids)})
                Order by Product.brand_id, Product.product_id
            """
            this.execute(getVendorURLQuery)
            url_list = this.fetchall()
            if url_list:
                logger.info(f"Found {len(url_list)} URLs to process")
                results = []
                for value in url_list:
                    product_id, product_mpn, brand_name = value
                    results.append((product_id, brand_name, product_mpn))
                return results
...
if __name__ == '__main__':
    ...
    try:
        ...
        vendors = [(10158,"https://hvacdiscountstore.com/")]

        mpns = getMpns(driver, 10158)

        for product_id, brand_name, mpn in mpns:
            for vendor_id, vendor_url in vendors:
                logger.info(f"Starting vendor: {vendor_id} | {vendor_url}")
                try:
                    logger.debug(f"Vendor {vendor_id} >> Processing product_id: {product_id}, mpn: {mpn}")
                    scraped_by_mpn(driver,vendor_id, vendor_url, product_id, brand_name, mpn)
                except Exception as e:
