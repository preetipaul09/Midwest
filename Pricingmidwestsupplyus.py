...
def scraped_by_mpn(driver,vendor_id, vendor_url, product_id, brand_name_from_DB, mpn, product_url, vendor_product_id):
    try:
        ...
        #         (By.CSS_SELECTOR, "div.product__price #ProductPrice")
        #     )
        # )
        ...
        # msrp_el = wait.until(
        # EC.presence_of_element_located(
        #         (By.CSS_SELECTOR, "div.product__price .ComparePrice strike")
        #     )
        # )
        ...
        try:
            # input_tab = driver.find_element(By.CSS_SELECTOR, "input.custom-form-control.prodoption.detailprodoption")
            # from selenium.webdriver.support.ui import WebDriverWait
            # first_input = WebDriverWait(driver, 10).until(EC.element_to_be_clickable((By.CSS_SELECTOR, "input.custom-form-control.prodoption.detailprodoption")))
            # first_input.click()
            # first_input.send_keys(mpn)
            ...
            wait.until(
                EC.visibility_of_element_located(
                    (By.CSS_SELECTOR, "a.btn.green.cart__cart")
                )
            )
            time.sleep(5)
            ...
            # Enter ZIP code
            zip_input.clear()
            zip_input.send_keys("11001")

            # Trigger change event (important for AJAX calculations)
            driver.execute_script("""
                arguments[0].dispatchEvent(new Event('change', { bubbles: true }));
                arguments[0].dispatchEvent(new Event('blur', { bubbles: true }));
            """, zip_input)

            logger.debug("ZIP code entered: 11001")

            state_dropdown = wait.until(
                EC.element_to_be_clickable((By.ID, "address_province"))
            )

            Select(state_dropdown).select_by_visible_text("New York")

            logger.debug("State selected: New York")

            # Wait a moment for shipping recalculation
            time.sleep(2)

            # If site requires Update Totals click after ZIP change
            update_button = wait.until(
                EC.element_to_be_clickable(
                    (By.CSS_SELECTOR, "input.get-rates.btn.gray.button")
                )
            )
            ...
            shipping_cost = driver.find_element(
                By.CSS_SELECTOR, "ul#shipping-rates li"
            ).text.replace("$","").replace("UPS Ground at","").replace("Free Shipping at","")

            price = float(price.replace(",", "").strip())
            shipping_cost = float(shipping_cost.replace(",", "").strip())
            ...
            #     By.CSS_SELECTOR, "#grandtotalspan"
            # ).text.replace("$","")
            ...
            remove_button = wait.until(
                EC.element_to_be_clickable(
                    (By.CSS_SELECTOR, "a.cart__remove")
                )
            )
